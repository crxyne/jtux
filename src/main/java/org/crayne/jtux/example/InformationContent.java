...
public class InformationContent extends Content {
    ...
    public void updateTime() {
        ...
        characterGrid().ifPresent(grid -> {
            ...
            grid.writeLineWrap(Vec2i.of(0, 2 + offsetY + offsetY2), Text.text(text2));
        });
    }

    public void run() {
        characterGrid().ifPresent(grid -> offsetY = grid.writeLineWrap(Vec2i.origin(), Text.text(ui.totalUpdates() + " full rendering updates (full terminal ui redraws)")));
        updateTime();
    }
